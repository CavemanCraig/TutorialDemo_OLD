...
public class MyWebService {
    ...
    public Player getPlayer(@PathParam("facebookID") long facebookID) {
        ...
        //Whenever we get a player now, we also want to re-generate a game link for them
        if (null!=foundPlayer){
    ...
    public Collection<User> getPlayersFriends(
        ...
        System.out.println("Returning Friend List, first one is: " + result.get(0));
        return result;
    }
    ...
    public Player playerPOSTRequest(
        ...
        if(null == foundUser){
            ...
            System.out.println("New User for PlayerInfo for Player request.  Adding User to the system.");
            User user = new User(facebookID, name, imageURL);
            em.persist(user);
            foundUser = user;
        }
        ...
        if (null == foundPlayer) {
            // NEW PLAYER
        	System.out.println("New Player request.  Adding Player to the system.");
 ...
 // Performs validation on answers submitted, and adjust points.
    ...
    public String postForAnswers(
        ...
        if(null==player || null==user1 || null==user2 || null==user3){
            //This should never happen, but lets exit gracefully if it somehow does.
        ...
        returnString += "You will have a total of [" + Math.abs(pointChange) + "] points ";
        if(pointChange > 0){
            returnString += "added!";
        }
        else{
    ...
    public static Player getPlayerByFacebookID(EntityManager em, long facebookID) {
        ...
        try {
            ...
            if (result.isEmpty()) {
                ...
                if (result.size() > 1) {
                    System.out.println("ERROR: We have [" + result.size()
                            + "] Players that have the FacebookID of ["
                            + facebookID + "].  I'll return the first one entered into the system");
                    return result.get(0);
                }
                else {
    ...
    public static void UpdateFriendsListForPlayer(EntityManager em,
        ...
        for (int i = 0; i < friendIDs.size(); i++) {
            ...
            if (null == friendUser) {
                ...
                // this player.
                System.out.println("New User needed to be added to the system. - "
                                + friendID + ", " + friendName + ", " + friendImageURL);
    ...
    private Link generateRandomizedGameLink(EntityManager em, Player player){
    	...
    	if(friendIDs.size() <= 4){
    		...
    		outputLink.setOnClickMethod("(function (){alert('You do not have enough friends to play the game.');return false;});");
        ...
        for (int index = 0; index <= 4; ++index){
        	...
        	//Use randomInt as the index in the friendIDs list as the next friend to use
        ...
        //Now that we have the 5 friends to use, we'll display the images of the first 3
    	...
    	outputLink.setHref("playGame.html?playerID=" + player.getPlayerInfo().getFacebookID() + 
